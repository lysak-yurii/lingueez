...
def _seed(path):
    ...
    cur.execute(
        "INSERT INTO words (ID, Word1, Word2, Status, cloud_id) VALUES (1,'cat','Katze','New',101)"
    )
    cur.execute(
        "INSERT INTO words (ID, Word1, Word2, Status, cloud_id) VALUES (2,'dog','Hund','New',102)"
    )
    ...
    cur.execute(
        "INSERT INTO bin_items (table_name, record_id, payload) VALUES ('words', 50, ?)",
        (json.dumps({"ID": 50, "Word1": "gone", "Word2": "weg"}),),
    )
    # pending sync state (should be cleared) + metadata.
    cur.execute(
        "INSERT INTO sync_queue (operation_type, table_name, record_id) VALUES ('INSERT','words',3)"
    )
...
class MigrationTests(unittest.TestCase):
    ...
    def test_full_migration(self):
        ...
        self.assertEqual(
            cur.execute("SELECT COUNT(*) FROM word_tags").fetchone()[0], 3
        )  # 4 - 1 orphan
        self.assertEqual(
            cur.execute("SELECT COUNT(*) FROM review_events").fetchone()[0], 4
        )  # incl orphan
        ...
        self.assertIsNone(
            cur.execute("SELECT value FROM sync_metadata WHERE key='last_sync_time'").fetchone()[0]
        )
        self.assertEqual(
            cur.execute("SELECT value FROM sync_metadata WHERE key='synced_account_id'").fetchone()[
                0
            ],
            "acc-123",
        )
    ...
    def test_idempotent(self):
        ...
        c, cur = self._conn()
        after = {t: cur.execute(f"SELECT COUNT(*) FROM {t}").fetchone()[0] for t in snapshot}
